...
#### 1. "ChromaDB has 0 documents" or "No supporting articles found"

**Cause:** Vector database is empty or not indexed.
...
#### 2. "ModuleNotFoundError: No module named 'dateparser'"
...
#### 3. "ValueError: Expected where to have exactly one operator, got {}"

**Cause:** ChromaDB version compatibility issue (fixed in latest code).

**Solution:** Update to latest code or ensure you're using the correct ChromaDB query syntax.

#### 4. "Analytics payload not found"

**Cause:** Analytics metrics not generated.
...
**Check logs:**
```bash
# In the terminal running uvicorn, look for the error traceback
# Common issues:
# - GOOGLE_API_KEY not set
# - ChromaDB path incorrect in config/rag.yml
# - Missing analytics files
```

#### 6. Q&A returns empty timeline

**Possible causes:**
- Articles are outside the date range (default: last 90 days)
- No articles match the query semantically
- Ticker/source filters too restrictive

**Solutions:**
- Expand date range: use `"days_back": 365` or `"natural_language": "all time"`
- Broaden your query
...
Real-time insights powered by sentiment analysis and network analysis.

### Available Metrics

1. **Sector Sentiment Over Time**
   - Aggregate sentiment scores by sector (Tech, Finance, Healthcare, etc.)
   - Time-series visualization showing sentiment trends
   - Powered by transformer-based sentiment models

2. **Ticker Mention Frequency**
   - Count of article mentions per ticker symbol
   - Filtered by date range and custom ticker lists
   - Identifies trending stocks in news coverage

3. **Ticker Co-occurrence Network**
   - Which tickers are mentioned together in articles
   - Heatmap visualization of co-mention frequency
   - Useful for understanding sector relationships and correlations

### Regenerating Analytics

After adding new articles or updating tickers:
...
**Outputs:**
- `overview.json` - Summary stats for dashboard
- `ticker_mentions.json` - Mention counts by ticker
- `ticker_cooccurrence.json` - Co-occurrence matrix
- `sector_sentiment.parquet` - Time-series sentiment data
- `sentiment.parquet` - Cached article-level scores
...
- Follow the minimal change policy (see `.github/copilot-instructions.md`)
...
- **OpenAI/Google Gemini** - Embeddings and LLMs
...
- [ ] Advanced visualizations (network graphs, word clouds)
